  ...
  if (!cart || cart.cartItems.length === 0) {
    return (
      <div className="container mx-auto py-12 px-4">
        <h1 className="text-2xl sm:text-3xl font-bold text-gold mb-6">Carrito de Compras</h1>
        <div className="bg-black/40 border border-gold/20 rounded-md p-8 text-center">
          <p className="text-gray-400">Tu carrito está vacío</p>
          <Button asChild className="mt-4 bg-gold hover:bg-gold/80 text-black font-medium">
              ...
              {cart.cartItems.map((item) => (
                <div key={item.id} className="flex gap-4 py-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-400">{item.type === 'card' ? 'Carta' : 'Paquete'}</p>
                    <div className="flex justify-between items-center mt-2">
                      ...
                      <div className="flex items-center gap-2">
                        <Button 
                          ...
                          onClick={() => cart.updateCartItemQuantity(item.id, -1)}
